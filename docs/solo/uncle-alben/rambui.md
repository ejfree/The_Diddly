---
title: Uncle Alben — Rambui
updated: 2026-08-01
tags: ["solo", "uncle-alben", "pf2e", "liquor", "tian-xia"]
---

# Rambui

> This page covers Alben's personal brewing process — how he actually *makes* his signature rambutan brandy. For the cultural origin of Rambui and its relationship to the foreign spirit "Drambuie" it mocks, see [background.md](background.md#rambui-the-orcish-brew). For the full hundred-bottle Cabinet of collected liquors and the drink-offering ritual, see [liquor.md](liquor.md).

---

## Where the Craft Comes From

Alben didn't invent Rambui, but he did formalize it. He grew up watching his jungle-orc hold ferment rambutan by the barrelful with no fixed recipe — just instinct, argument, and whoever's batch made the elders wince hardest that year. It was during his years as a barkeep, hauling casks and keeping a professional bar's stock consistent night after night, that he started treating it like an actual process instead of a seasonal free-for-all: measured mash times, a repeatable "ruining" step, and batches he could reliably reproduce on the road once he traded the tap room for a maul.

He never wrote any of it down. It lives in his hands, his nose, and his **Lore: Alcohol** skill (Trained, +13) — the same skill that lets him talk shop with any bartender, distiller, or brewer he meets, and the one the ritual below runs on.

---

## Ingredients & Preparation

1. **The Harvest** — Ripe rambutan fruit, husked and mashed, pit and all (the pit is where the real bitterness lives, and bitterness is the point). Jungle-orc tradition insists on fruit that's just started to turn, never perfectly ripe — a "proper" Rambui should taste like it was made from fruit that couldn't be bothered to wait.
2. **The Ferment** — Mash goes into a sealed cask with wild yeast (never a cultivated strain — cultivated yeast makes things smooth, and smooth is the enemy) for several days until it's aggressively alive.
3. **The Distillation** — A single, rough pass through a still, deliberately under-refined. Where a proper Drambuie-style brandy would be distilled slow and clean to preserve the honeyed notes, Rambui is rushed and a little dirty on purpose, carrying over exactly the harsh, fusel notes a careful distiller would normally discard. In most distilleries, the 'Head' of the final product is discarded. In Alben's Rambui, it is the heart. During the heart of the distilation, boring is enouraged. Finally the hammer in the tail as the final distallates strip the flesh from your throat, though subtually...to an orc. 
4. **The Ruining** *(Alben's signature step)* — Before casking, he adds a handful of charred cask stave shavings and a pinch of ground rambutan pit back into the spirit. This is the step his old hold considered optional and Alben considers mandatory: it's what pushes a drinkable brandy into a proper Rambui. He jokes that this is the only step Drambuie's original distillers would recognize as a *mistake*.
5. **The Cask** — Aged in whatever barrel is on hand, ideally one that's held Rambui before (a "virgin" barrel is considered wasted potential). Alben keeps a rotating set of travel-worn casks in his **Spacious Pouches** (see [inventory.md](inventory.md)) — they don't accelerate aging, but they let him carry a whole rotating cellar's worth of batches at various stages without adding a scrap of carried Bulk.
6. **The QA Tester** — Before any batch is declared finished, it goes to **Solomon**. Alben pours a single drop onto a small dish and sets it in front of the duck, who inspects it with the same unbothered dignity he brings to everything else. A batch Solomon drinks without hesitation passes. A batch Solomon sniffs, considers, and pointedly ignores gets sent back for another round of Ruining. Alben insists this isn't superstition — "the bird's got a nose the collar only sharpens" — and refuses to bottle a single batch Solomon hasn't signed off on, no matter how good Alben's own tongue tells him it is. So far, no one's ever caught Solomon getting the call wrong.

---

## Crafting Mechanics (Using Lore: Alcohol)

**Reasoning — **Lore: Alcohol substitutes for Crafting** whenever Alben brews, ages, or replicates a batch of Rambui specifically (not general alchemical crafting — just his own signature drink and close variants of it). This follows the same spirit as the general Lore-skill guidance that lets a sufficiently specific Lore substitute for Crafting within its own narrow field.

### Downtime Activity: Brewing a Batch

- **Skill:** Lore: Alcohol (+13, Trained)
- **Activity:** Craft (downtime), using Lore: Alcohol in place of Crafting
- **Time:** 4 days per batch (standard Craft activity timing)
- **DC:** 14 (standard DC for a level-0 alchemical item), +2 if brewing away from a proper cellar/still (i.e., using field equipment or the Spacious Pouches' travel casks instead of a real distillery)
- **Cost:** Negligible — rambutan fruit and basic cask materials are cheap and plentiful wherever jungle orc holds or comparable fruit markets exist; treat raw materials as effectively free for a single batch (GM may charge a token 1–2 gp for materials if Alben is somewhere rambutan doesn't grow)
- **Batch size:** One Craft activity produces a working batch of roughly 10 doses (i.e., 10 individual Alcohol items per successful batch)

### Degrees of Success

The result of the check determines which of Alben's three named Rambui products (see [inventory.md](inventory.md)) the batch becomes:

| Result | Product | Batch Size | Notes |
|--------|---------|-----------|-------|
| **Critical Success** | **Rambui — The Last Drop** | 2–3 doses | The best Rambui is judged on one axis only: harshness. This is **harsher** — the fermentation, distillation, and Ruining all landed exactly right, and the result is the most brutal, punishing pour Alben has ever produced. Trophy-tier, not stock; reserved for special occasions, dying friends, and the rare moment he wants to floor someone with how bad a drink can taste and still be *good*. |
| **Success** | **Rambui** | 10 doses | Proper Rambui: **harsh**. Every rough edge intact, exactly as harsh as it's supposed to be — no more, no less. This is the standard result, the bulk of his running stock, and "the usual" poured every time someone takes him up on a drink (see [liquor.md](liquor.md#the-ritual-lets-have-a-drink)). |
| **Failure** | **Emergency Rambui** | 6 doses | **Nothing.** Not smooth, not harsh — just flat and unremarkable, a batch that never found its bite at all. Drinkable, technically Rambui, but the kind of thing Alben quietly moves to the backup stash instead of the main rotation, because a Rambui with no bark and no bite has no business calling itself Rambui. |
| **Critical Failure** | *Ruined batch* | 0 doses | Every rough edge fermented, distilled, and ruined itself smooth instead — the Ruining step failed to take, and what comes out is honeyed, mellow, and perfectly balanced. In other words, it's basically Drambuie again. To Alben, that makes it genuinely undrinkable: no bite, no bark, nothing to grimace through. He can't in good conscience call it Rambui, and the materials are wasted on what amounts to a very expensive, very disappointing sipping brandy. Alben takes this personally — a critical failure means he's out of practice, distracted, or brewing somewhere with genuinely poor conditions, and he'll usually mutter about it for the rest of the day, occasionally forcing a taste on someone just to confirm he isn't losing his touch. |

---

## The Legend of HFCS

Somewhere between his third and ninth drink one night, years back, a traveling merchant swore to Alben that there existed a substance so potent, so refined, so unnaturally sweet that a single drop could reshape an entire batch — outsiders apparently called it, in hushed and reverent tones, **HFCS**. The merchant didn't know what it stood for. Neither does Alben. Nobody Alben has ever asked knows what it stands for, which as far as he's concerned only proves how real it must be.

He's chased rumors of it since — a syrup, thick and gold and impossibly sweet, said to be brewed in vats the size of a keep by people who don't exist in any hold or city he can name. Some tellings claim it's squeezed from a corn so distant and strange it isn't corn at all. Others claim it isn't natural in the slightest, and that whoever first made it did so somewhere no orc, elf, or dwarf has ever set foot. Alben doesn't put much stock in any one version. He puts stock in the idea that if something can be *that* sweet, then whatever Ruining does to sweetness, HFCS would let him take further than he's ever taken anything.

He has never seen it, tasted it, or met anyone who has. He's not entirely convinced it isn't a story bartenders tell each other to sound well-traveled. That hasn't stopped him from asking after it in every port, hold, and back-alley still he's ever passed through — a standing, half-joking question at the end of every trade negotiation for ingredients: *"You wouldn't happen to have any HFCS, would you?"* Nobody ever does. Someday, Alben is certain, somebody will — and on that day, he intends to Ruin the finest batch of Rambui the world has ever tasted.

**GM Note:** HFCS is presented here purely as an in-world legend/rumor — an anachronistic real-world reference (high-fructose corn syrup) reframed as a mythical brewing ingredient Alben has heard of but can't identify or source. Treat it as a standing hook: a GM could seed an actual (mundane or magical) substance into the setting for Alben to eventually find, use it as a running joke NPCs bring up to bait him, or leave it as an unresolved legend that colors his character voice. It should not be treated as an existing, defined item unless and until it's given a concrete answer.

---

## The Ritual Loop

Brewing isn't just a hobby — it's what keeps "Let's have a drink" (see [liquor.md](liquor.md#the-ritual-lets-have-a-drink)) supplied. Every time Alben pours "the usual," he's drawing down the standing stock that his downtime brewing replenishes. A long stretch without downtime to brew is one of the few things that visibly worries him more than a fight he can't win — running out of Rambui to offer is, to Alben, a small personal failure of hospitality.

---

## Cross-References

- [background.md](background.md) — Full cultural background on Rambui and Alben's relationship to it
- [liquor.md](liquor.md) — The Liquor Cabinet, drink-offering ritual, and mechanics shared by all alcohol items
- [inventory.md](inventory.md) — Current Rambui stock and full inventory
- [character.md](character.md) — Full mechanical character sheet, including Lore: Alcohol
- [concept.md](concept.md) — Original character concept and voice
