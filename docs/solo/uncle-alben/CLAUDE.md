...
- **⚙ Automatic Bonus Progression is in use.** Fundamental runes (potency, striking, resilient) **do not exist** in this campaign — their equivalents are granted automatically by level. Property runes (flaming, etc.) still work normally and no longer need a potency rune. **Never add, price, or recommend a fundamental rune here.** At level 12 Alben has Attack Potency +2, Devastating Attacks (3 weapon damage dice), Defense Potency +2, Save Potency +1, Perception Potency +1, and three **unassigned** Skill Potency picks. Full table in `character.md` → Automatic Bonus Progression.
- **AC 32**, achieved via **Warpriest's Armor** (class feat) granting heavy armor training, worn with Full Plate, plus ABP Defense Potency. HP 166; Spell DC 30 (ABP grants casters nothing); Perception +21; Fort +21 / Ref +17 / Will +23.
- **Signature weapon:** Flaming Maul, **+22 for 3d12+6 plus 1d6 fire** — its head cast/carved to resemble a squat brandy cask (barrel banding, bunghole detail) rather than a plain hammer block. Under ABP the flaming property rune is the only rune on it, and the only kind worth buying for it.
- **Build:** **6 ft 9 in, 430 lb** (2.06 m, 195 kg) — tall even for an orc (PF2e Player Core: orcs commonly top 7 ft) and deliberately fat as well as enormously strong; the model is a heavyweight sumo / strongman, not a sedentary body. By modern human standards that's a **BMI ~46, Class III obesity** — intentional, but a human metric applied to an orc frame, and never framed in-world as a medical issue. See `background.md` → Build & Physicality.
...
- **Key items:** Collar of Empathy, Flask of Fellowship, **Holy Prayer Beads (Greater)**, **Aeon Stone (Pearly White Spindle)**, two Spacious Pouches (Type I) — #1 holds the bar, #2 holds scrolls and the backup Rambui — plus a new, still-empty **Spacious Pouch (Type III)** (100 Bulk).
- **Free Archetype:** he takes both a class feat and an archetype feat at every even level. Assume that variant rule is in play.
...
- **FoundryVTT exports** land in `assets/foundryvttjson/` (most recent: **`fvtt-Actor-uncle-alben-9MuIf1CmSuTUk4QX.json`**, level 12; prior: `unclealben11v1.json`, level 11). When a new export is provided, reconcile it against `character.md`, `spells.md`, and `inventory.md`, calling out every change via GM Notes rather than assuming. Each of those three files carries a "GM Note — Changes from prior export" section; extend it rather than replacing it.
- **Level-dependent numbers appear outside the sheet files too** — `plans/actionplans.md` has a Baseline numbers block, and `background.md` and `church/bigchurch.md` both quote his Athletics modifier. Grep the folder for the old values after any level change.
