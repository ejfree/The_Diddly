---
title: G1 — The Way-Station
updated: 2026-03-14
tags: []
---

# G1 — The Way-Station

## **The Way-Station**
A permanent drow logistics post — a carved-out cavern used as a supply depot, barracks, and patrol coordination point. Multiple houses run patrols through here; the commander is House Xilrath.

### **Description**
A permanent drow logistics post — a carved-out cavern used as a supply depot, barracks, and patrol coordination point. Multiple houses run patrols through here; the commander is House Xilrath.

### **Features & Terrain**

- **Lighting**:

- **Surfaces**: Carved-out cavern; worked stone

- **Objects**: Carved stone notice-board with patrol schedules, missing slave reports, and a recent edict from Matron Vel'dryn ordering heightened surface-intruder watch (posted 5 days ago); caged quaggoth awaiting pickup by Dravun

- **Doors & Exits**:

- **Special Interactions**: Intel board is visible and readable — patrol schedules, missing slave reports, and the recent heightened-alert edict

### **Smells & Sounds**

- **Smells**:

- **Sounds**: Barracks activity; patrol movement

### **Inhabitants**

- **Passive Creatures**: 1 caged quaggoth awaiting pickup by Dravun

- **Hostile Threats**: 8 drow soldiers (Xilrath), 2 Xilrath priestesses

- **Unique NPCs**: 1 senior sergeant (Lvl 7 Fighter)

### **Traps & Hazards**

- **Trap Name & Type**:

- **Trigger**:

- **Effect**:

- **Disarm**:

### **Loot & Rewards**

- **Visible Loot**: Ration packs, spare manacles, torch-chalk (drow marking wax)

- **Hidden Loot**:

- **Magical Items**:

### **Additional Notes**
Intel Board: A carved stone notice-board with patrol schedules, missing slave reports, and a recent edict from Matron Vel'dryn ordering heightened surface-intruder watch (posted 5 days ago — they're expecting trouble).

### **Midjourney Prompt**

drow military logistics post carved into underdark cave, supply depot with equipment racks and ration packs, carved stone notice-board on wall covered in drow script, caged creature in corner, dim torch light, drow black stone architecture, organized dark elf military facility, fantasy RPG background --ar 16:9 --v 6.1
